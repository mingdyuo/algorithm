...
> 21.03.13 **실패**
>
> *~~와!!!!!!!!!!!!!!!!!!!!!!! 이거 시뮬레이션 인가? 했는데 BFS엿음 ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ~~*
>
> BFS 초반에 입문 문제같은거 풀면 이런 문제 있는데, 또 형식을 뭔가 복잡스럽게 바꿔놓으니까 문제를 딱 봤을 때 전혀 감이 안왔다. 문제 생긴거 보고 판단 하지 말자

- [ ] **7662 이중 우선순위 큐**

  > 21.06.01 **실패**
  >
  > PQ를 두개 사용하는 것은 맞았으나, 유효한 (반대편에서 꺼낸 원소인지 확인하는) 나의 논리가 완벽하지 않았음
  >
  > 나는 1) `pq_asc.top() > pq_dsc.top()` 2) `pq_asc.empty()` 3) `pq_dsc.empty()`인 경우에 다 비우도록 하고, `pq_asc.top() == pq_dsc.top()`인 경우에는 서로 `pop()`하도록 했다. 
  >
  > 사실 이 방식에 스스로도 완전한지 반신반의 했는데, 더 나은 방식은 `Insert`할 때 인덱스 값을 부여하여서, 해당 값이 삭제되었는지 체크하는 방식이다. 
  >
  > 이렇게 바꾸어서 통과하였다. 